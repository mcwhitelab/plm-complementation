...
class SelfAttentionPooling(nn.Module):
    """
    Self-attention pooling for protein sequence embeddings.
    
    This implements the simpler self-attention mechanism described in the paper:
    attention-pooled = (1/l) * sum(attention_weights * original_embeddings)
    
    Where attention_weights come from softmax(Q(x) @ K(x)^T) and Q, K are learnable
    nonlinear transformations.
    
    Args:
        dim (int): Input embedding dimension
        inner_hidden_dim (int, optional): Dimension for query/key attention computation.
                                         Defaults to min(256, dim // 4)
    """
    def __init__(self, dim, inner_hidden_dim=None):
        super(SelfAttentionPooling, self).__init__()
        self.dim = dim
        
        # Auto-adjust inner dimension based on input size
        if inner_hidden_dim is None:
            self.inner_hidden_dim = min(256, dim // 4)  # Adaptive bottleneck
        else:
            self.inner_hidden_dim = inner_hidden_dim
        
        # Learnable Q and K networks (no V network needed)
    ...
    def forward(self, x):
        ...
        qx = self.qnet(x)  # [batch_size, seq_length, inner_hidden_dim]
        kx = self.knet(x)  # [batch_size, seq_length, inner_hidden_dim]
        
        # Compute attention weights: softmax(Q(x) @ K(x)^T)
        attention_scores = qx @ kx.transpose(-1, -2) / (self.inner_hidden_dim ** 0.5)
        attention_weights = F.softmax(attention_scores, dim=-1)
        
        # Apply attention to original embeddings (no separate value network)
        # This is the key difference from full QKV attention
        attended_values = attention_weights @ x  # [batch_size, seq_length, hidden_dim]
        
        # Average across sequence length: (1/l) * sum(...)
        return attended_values.mean(dim=1)  # [batch_size, hidden_dim]
